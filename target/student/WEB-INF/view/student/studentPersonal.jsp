	    	...
	    	buttons: [
	  	    		{
	  					...
	  					handler:function(){
	  						...
	  						if(!validate){
	  							...
	  							$.ajax({
	  								type: "post",
	  								url: "${basePath}/system/editPasswod",
	  								data: $("#editPassword").serialize(),
	  								success: function(msg){
	  									if(msg == "success"){
	  										$.messager.alert("消息提醒","修改成功，将重新登录","info")
	  										setTimeout(function(){
	  											top.location.href = "${basePath}/login/loginOut";
	  										}, 1000);
	  									}else if(mag="error"){
											$.messager.alert("消息提醒","原密码错误","info")
										}else{
											$.messager.alert("消息提醒","修改失败","info")
										}
	  								}
	  							});
	  						}
	    	...
	    	toolbar: [
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							$.ajax({
								type: "post",
								url: "${basePath}/student/editStudent2",
								data: $("#editForm").serialize(),
								success: function(msg){
									if(msg == "success"){
										$.messager.alert("消息提醒","更新成功!","info");
										setTimeout('parent.location.reload()',1500); //指定1秒刷新一次
									} else{
	...
	<div id="editDialog" style="padding: 20px">
    	...
    	<form id="editForm">
	    	<table cellpadding="8" >
	    		...
	    		<tr>
	    			<td>年级:</td>
	    			<td><input id="edit_grade"  value="${userDetail.clazzName}" style="width: 200px; height: 30px;" class="easyui-textbox" data-options="readonly: true"/></td>
	    		</tr>
	    		<tr>
	    			<td>班级:</td>
	    			<td><input id="edit_clazz"  value="${userDetail.gradeName}" style="width: 200px; height: 30px;" class="easyui-textbox" data-options="readonly: true"/></td>
	    		</tr>
	    	</table>
	    </form>
	</div>
    	...
    	<form id="editPassword">
	    	<table cellpadding="8" >
	    		<tr>
	    			<td>原密码:</td>
	    			<td>
	    				<input id="old_password" style ="width: 200px; height: 30px;" class="easyui-textbox" type="password" name="oldPassword"   data-options="required:true, missingMessage:'请输入原密码'" />
	    			</td>
	    		</tr>
	    		<tr>
	    			<td>新密码:</td>
