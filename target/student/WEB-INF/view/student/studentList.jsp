...
<head>
	...
	<script type="text/javascript" src="${basePath}/easyui/js/page.js"></script>
	...
	$(function() {	
	    ...
	    $("#delete").click(function(){
        	...
        	if(selectLength == 0){
            	...
            	$.messager.confirm("消息提醒", "将删除"+selectLength +"位学生相关的所有数据(包括成绩)，确认继续？", function(r){
            		if(r){
            			$.ajax({
							type: 'post',
	...
	//按学号或者姓名查找学生
	function studentQueryBtn() {
		var number=$("#number").val();
		var name=$("#name").val();
		$.ajax({
			method : 'post',
			url : '${basePath}/student/getStudentByName',
			data:{
				"page":$("#dataList").datagrid('options').pageNumber,
				"rows":$("#dataList").datagrid('options').pageSize,
				"number":number,
				"name":name
			},
			async : false,
			...
			success : function(data) {
				if(data.total<1){
					$.messager.alert("消息提醒","没有找到相关的信息!","info");
				}
				$('#dataList').datagrid('loadData', data);
			},
			error : function() {
				$.messager.alert("消息提醒","错误!","info");
			}
		});
}
...
<body>
	...
	<div id="toolbar">
		...
		<div style="float: left; margin: 0 10px 0 10px">班级：<input id="clazzList" class="easyui-textbox" name="clazz" /></div>
		<div id="tb" style="margin-left:10px;">
				 学号: <input class="easyui-textbox" type="text" name="student.number"  id="number" data-options="prompt:''" /> 
				 姓名: <input class="easyui-textbox" type="text" name="student.name" id="name" data-options="prompt:'可输入姓名关键字'"/> 
				<a id="studentQueryBtn" href="javascript:void(0)"  onclick="studentQueryBtn()" class="easyui-linkbutton" data-options="iconCls:'icon-search'">搜索</a>
		</div>
	</div>
	...
	<div id="addDialog" style="padding: 10px">  
		<div style="float: right; margin: 20px 20px 0 0; width: 200px; border: 1px solid #EBF3FF" id="photo">
	    	<img alt="照片" style="max-width: 200px; max-height: 400px;" title="照片" src="${basePath}/photo/student.jpg" />
	    </div> 
