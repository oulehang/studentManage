...
<%--<script type="text/javascript">--%>
	<%--$(function(){--%>
		...
		<%--$("#vcodeImg").click(function(){--%>
			<%--this.src="/login/getVCode?time="+new Date().getTime();--%>
		<%--});--%>
		<%----%>
		<%--//登录--%>
			...
			<%--$.ajax({--%>
				...
				<%--success: function(msg){--%>
					...
					<%--} else if("loginError" == msg){--%>
						...
						<%--$("input[name='vcode']").val("");//清空验证码输入框--%>
					<%--} else if("admin" == msg){--%>
						<%--window.location.href = "/login/adminView";--%>
					<%--} else if("student" == msg){--%>
						<%--window.location.href = "/login/studentView";--%>
					<%--} else if("teacher" == msg){--%>
						<%--window.location.href = "/login/teacherView";--%>
					<%--}--%>
				<%--}--%>
				<%----%>
			<%--});--%>
    ...
    <%--<form id="form" class="form form-horizontal" method="post">--%>
      ...
      <%--<div class="row cl">--%>
        <%--<div class="formControls col-8 col-offset-3">--%>
          ...
          <%--<img title="点击图片切换验证码" id="vcodeImg" src="/login/getVCode"></div>--%>
      <%--</div>--%>
      <%----%>
